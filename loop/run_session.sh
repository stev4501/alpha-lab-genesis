...
# ---------- validate ----------
if loop/validate_session.sh main "$BRANCH" "$SESSION_ID" > "$LOGDIR/validate.log" 2>&1; then
  log "Validation PASSED -> merging to main."
  git checkout -q main
  git merge -q --no-ff -m "session ${SESSION_ID} (${MODE}, agent_exit=${AGENT_EXIT})" "$BRANCH"
  git branch -q -D "$BRANCH"
  git push -q origin main || log "WARN: push failed; will retry next session."
else
